...
|---|---|---|---|---|---|---|---|
| TC-05-01 | Return a borrowed book successfully | Member `MEM002 (Nguyễn Học Bá)` is logged in. Book `BOOK003` is currently borrowed by the member. | 1. Open **Borrow / Return** page.<br>2. Locate borrowed record `BR001`.<br>3. Click **Return Book**.<br>4. Confirm return action. | `BR001`, `BOOK003` | Book is returned successfully. Book status changes to **"Available"**. Borrow record status changes to **"Returned"**. | REQ-05 | EP |
| TC-05-02 | Return a book before overdue date | Member `MEM002` is logged in and has an active borrow record. `returnDate < dueDate`. | 1. Open **Borrow / Return** page.<br>2. Select an active borrow record.<br>3. Click **Return Book**. | `returnDate = 14/06/2026`, `dueDate = 15/06/2026` | Book is returned successfully. No overdue warning message is displayed. Book status changes to **"Available"**. | REQ-05 | EP, BVA |
| TC-05-03 | Return a book on the due date (boundary case) | Member has an active borrow record. `returnDate = dueDate`. | 1. Open **Borrow / Return** page.<br>2. Select an active borrow record.<br>3. Click **Return Book**. | `returnDate = 15/06/2026`, `dueDate = 15/06/2026` | Book is returned successfully. System displays a **clear overdue warning**, because `dueDate <= today` is considered overdue. | REQ-05 | BVA |
| TC-05-04 | Return an overdue book | Member has an active borrow record. `returnDate > dueDate`. | 1. Open **Borrow / Return** page.<br>2. Select overdue borrow record.<br>3. Click **Return Book**. | `returnDate = 16/06/2026`, `dueDate = 15/06/2026` | Book is returned successfully. System displays a **clear overdue warning**. Book status changes to **"Available"**. Borrow record status changes to **"Returned"**. | REQ-05 | EP, BVA |
| TC-05-05 | Member cannot access borrow records of other members | Member `MEM002` is logged in. Another member (`MEM003`) has borrow records. | 1. Login as `MEM002`.<br>2. Open **Borrow / Return** page.<br>3. Observe displayed borrow records. | `MEM002`, records of `MEM003` | Member only sees their own borrow records. Borrow records of other members are not displayed and cannot be returned. | REQ-05, REQ-08 | EP |

## Test Cases — REQ-06: Overdue Handling

| TC ID | Test Objective | Preconditions | Test Steps | Input Data | Expected Result | REQ | Technique |
|---|---|---|---|---|---|---|---|
| TC-06-01 | Check borrow record not overdue | Librarian is logged in. A borrow record exists with `dueDate > today`. | 1. Open **Borrow Records Management** page.<br>2. Click **Check Overdue** button.<br>3. Observe borrow record status. | `today = 15/06/2026`, `dueDate = 20/06/2026` | Borrow record is **not marked as "Overdue"**. Status remains **"Borrowing"**. | REQ-06 | EP, BVA |
| TC-06-02 | Check borrow record due today (boundary case) | Librarian is logged in. A borrow record exists with `dueDate = today`. | 1. Open **Borrow Records Management** page.<br>2. Click **Check Overdue** button.<br>3. Observe borrow record status. | `today = 15/06/2026`, `dueDate = 15/06/2026` | Borrow record is marked as **"Overdue"**, because `dueDate <= today` is considered overdue. | REQ-06 | BVA |
| TC-06-03 | Check overdue borrow record | Librarian is logged in. A borrow record exists with `dueDate < today`. | 1. Open **Borrow Records Management** page.<br>2. Click **Check Overdue** button.<br>3. Observe borrow record status. | `today = 15/06/2026`, `dueDate = 10/06/2026` | Borrow record is marked as **"Overdue"**. | REQ-06 | EP, BVA |
